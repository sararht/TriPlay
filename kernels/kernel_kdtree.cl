...
struct BoundingBox
{
    struct vector3D minPos;
    struct vector3D maxPos;
};
struct KDNode
{
    struct KDNode *left;
    struct KDNode *right;
    struct TriangleArray *triangles;
...
kernel void frst_prog(__global struct KDNode* node,
...
float jaja = node->bbox.maxPos.x; //[0]<-problemaa
//struct KDNode *nodenode;
//nodenode->triangles->tris->v0.x=1;
...
/*
float3 v0_ = {node->tris[gid]->v0.x,node->tris[gid]->v0.y,node->tris[gid]->v0.z };
float3 v1_ = {node->tris[gid]->v1.x,node->tris[gid]->v1.y,node->tris[gid]->v1.z };
float3 v2_ = {node->tris[gid]->v2.x,node->tris[gid]->v2.y,node->tris[gid]->v2.z };
