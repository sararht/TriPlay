...
//      //  std::string path_global = "/home/sara/Descargas/puerta_pequena2/";
//        std::string path_global = "/home/sara/Descargas/penholder/"; //portaRotu
...
//         //   printQVector3D(mean_n.toQVector3D().normalized());
...
//        //----------------------------------
//        //Derivar la posición y buscar los puntos clave!
//        QVector<double> pos_x, pos_y;
//        for (const auto& point : new_sensor_position) {
//            pos_x.push_back(point.z());
//            pos_y.push_back(point.y());
//        }

//        std::cout <<"1"<<std::endl;
//        // Filtro de derivada de la gaussiana
//        double sigma = 5.0; // desviación típica

//        // Rango de valores válidos de x
//        double range = ceil(3 * sigma);
//        cv::Mat x = cv::Mat::zeros(1, 2 * range + 1, CV_64F);
//        for (int i = -range; i <= range; i++) {
//            x.at<double>(i + range) = 1.0*i;
//        }
//        std::cout <<"2"<<std::endl;

//        // Filtro de Gaussiana sin el factor de normalización
//        cv::Mat g = cv::Mat::zeros(1, 2 * range + 1, CV_64F);
//        cv::Mat aux_mat = -(x.mul(x) / (2.0 * sigma * sigma));
//        cv::exp(aux_mat, g);

//        // Derivada primera en la dirección de X
//        cv::Mat aux2 = -(x.mul(x) / (2.0 * sigma * sigma));
//        cv::Mat aux3 = (-x / (sigma * sigma));
//        cv::Mat aux4;
//        cv::exp(aux2, aux4);
//        cv::Mat g1 = aux3.mul(aux4);
//        std::cout <<"3"<<std::endl;


//        // Calcula la derivada primera de pos_y respecto a pos_x

//        std::cout << "Size: " << pos_y.size() << std::endl;
//        cv::Mat proyh = cv::Mat::zeros(1,int(pos_y.size()), CV_64F);//, proyh_dgx;
//        for (int j = 0; j < proyh.cols; j++) {
//            proyh.at<double>(j) = -pos_y[j];
//        }

//        std::cout << proyh.rows << ", " << proyh.cols << std::endl;

//        std::cout <<"4.1"<<std::endl;

//        cv::Mat proyh_dgx = proyh.clone();
//        std::cout <<"4.2"<<std::endl;




//       // cv::Mat proyh_dgx = cv::Mat::zeros(1,pos_y.size(), CV_64F);
//        //QVector<double> proyh_qvector(pos_y.size());
//        //std::copy(pos_y.begin(), pos_y.end(), proyh_qvector.begin());
//        //cv::flip(proyh_qvector, proyh, 0);

//        cv::filter2D(proyh, proyh_dgx, -1, g1, cv::Point(-1, -1), 0, cv::BORDER_DEFAULT);

//        for (int i=0; i<proyh_dgx.cols; i++)
//            std::cout << proyh_dgx.at<double>(i) <<", ";
//        std::cout << std::endl;
...
//        // Encuentra picos en la derivada
//        std::vector<int> peaks;
//        for (int i = range; i < proyh_dgx.cols - range; ++i) {
//            double value = proyh_dgx.at<double>(i);
//            if (value > proyh_dgx.at<double>(i - 1) && value > proyh_dgx.at<double>(i + 1)) {
//                peaks.push_back(i);
//            }
//        }

//        // Imprime las posiciones de los picos
//        cout << "Picos encontrados en las posiciones: ";
//        for (int peak : peaks) {
//            cout << pos_x.at(peak) << " ";
//        }
//        cout << endl;
...
////----------------------------------------------
////Actualizar esas posiciones según normales:
...
//     QVector<QVector3Dd> ideal_sensor_position;
//     QVector<QVector3Dd> ideal_sensor_orientation;
//     for(int p=0; p<new_sensor_position.size(); p++)
//     {
//         QVector3Dd ideal_pos_aux = new_sensor_position[p];
//         QVector3Dd ideal_or_aux = new_sensor_orientation[p];

//        // printQVector3D(new_normals[p].toQVector3D());

//         ideal_pos_aux = (new_sensor_position[p] + new_normals[p].normalized()*50) /*working_distance*/;
//         ideal_or_aux.setY(90+angles_deg[p]);

//         ideal_sensor_position.push_back(ideal_pos_aux);
//         ideal_sensor_orientation.push_back(ideal_or_aux);
...
//        saveTraj(/*"/home/sara/Descargas/TRAJ/"*/path_global.c_str(),
//                 path_save2.c_str(),ideal_sensor_position, ideal_sensor_orientation); //new_rpy_sensor_orientation
